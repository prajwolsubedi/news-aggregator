...
def run_pipeline() -> Tuple[Response, int]:
    ...
    try:
        ...
        if success:
            ...
            logging.error("  1. Email sending failed (check RESEND_API_KEY and SENDER_EMAIL)")
            logging.error("  2. Resend API key invalid or expired")
            logging.error("  3. Domain not verified in Resend dashboard")
